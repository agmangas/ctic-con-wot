...
config_file_path = os.path.abspath(
    os.path.join("common_modules", "mqttClient", "config.json")
)
...
class MqttClient:
    ...
    def connect(self, topic_to_subscribe=None):
        """Connect and subscribe to topic (or topics) pass as parameter
        (If topic_to_subscribe is none, will subscribe to all the topics inside config.json file)"""
        ...
        try:
            ...
            if self.transport == "websockets":
                self._client.ws_set_options(headers={"Host": self.broker})
    ...
    def _subscribe(self, topics_to_subscribe):
        """Subscribe to a single topic or a list of topics pass as parameter.
        If parameter is None, will subscribe to all config's topics"""
    ...
    def create_from_environment():
        ...
        client_id = os.environ.get(
            "MQTT_CLIENT_ID", "client_" + str(random.randint(0, 1000))
        )
